...
export default class Dust extends Command {
    ...
    static override flags = {
        ...
        minAmount: Flags.string({description: 'minimum amount of token the recipient must have received in wei to be dusted', default: "0"}),
    }

    public async run(): Promise<void> {
        const { flags } = await this.parse(Dust)
        const { amount, chainId, privateKey, token, minAmount } = flags as {
            amount: string,
            chainId: number,
            privateKey: Hex,
            token: Address,
        ...
        this.log(`Dusting ${token} receivers on ${chainId} with minimum transfer amount of ${minAmount}`)
    }
}
